...
#4. Add Yuumi and Qiyana into num2words list
...
num2words = {1: 'Annie', 2: 'Olaf', 3: 'Galio', 4: 'Twisted Fate', 5: 'Xin Zhao', \
			...
			131: 'Diana', 518: 'Neeko', \
			143: 'Zyra', 145: "Kai'Sa", 350: 'Yuumi', 246: 'Qiyana', \
...
async def leaguerank(ctx, type, *, summonerName): #takes the player's name as parameter on discord cmd (space after cmd name)
	summNameInput = str('"{}"'.format(summonerName))
	try:
		summonerID = nameToID(summonerName)
		url = 'https://na1.api.riotgames.com/lol/league/v4/entries/by-summoner/'+summonerID+'?api_key=' + RIOTAPI #now grabs the ranked stats url and presents it
		response = requests.get(url) #sets a variable for the intake of the ranked stats
		nameExists = True
	except:
		await client.say('This name is not accepted or does not exist.')
		nameExists = False
	if (nameExists == True):
		try:
			if (type == 'solo' or type == 's' or type == 'sq' or type == 'soloq'):
			...
			elif (type == 'flex' or type == 'f' or type == 'team' or type == 't' or type == 'flexq'):
				tier = response.json()[1]['tier'] #takes the zero'th section of the array presented by the json file and finds 'tier' within it
				rank = response.json()[1]['rank'] #same thing but for 'rank' in the same section
				wins = response.json()[1]['wins']
				losses = response.json()[1]['losses']
				LP = response.json()[1]['leaguePoints']
				#position = response.json()[1]['position']
